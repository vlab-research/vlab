...
export const createInitialState = (confKey: string, conf: any) => {
  const { fields } = conf;
  ...
  const stateObj = fields
    ? {
        [confKey]: Object.assign({}, ...mapped),
      }
    : {
        [confKey]: {
          [nestedConfKey]: Object.assign({}, ...mapped),
        },
      };
...
export const createStateFromTuple = (tuple: string | any[]) => {
  ...
  const conf = tuple[1];
  return createInitialState(confKey, conf);
};
