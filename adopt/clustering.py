...
def make_reqs(target_key, surveys):
    return [
        (s["shortcode"], s["cluster_question"]["ref"], _make_reqs(s[target_key]))
        for s in surveys
    ]
...
def add_cluster(surveys, df):
    ...
    return (
        df.groupby("shortcode")
        .apply(
            lambda df: add_res_cols(
                [("cluster", reqs[df.shortcode.iloc[0]], lambda x: x)], df
            )
        )
...
def only_target_users(df, surveys, target_key, fn=users_fulfilling):
    ...
    users = [fn(tqs, df[df.shortcode == sc]) for sc, cq, tqs in reqs]
...
def _saturated_clusters(stratum, targets):
    ...
    clusters = (
        targets.groupby("cluster").filter(is_saturated)["cluster"].unique().tolist()
    )
