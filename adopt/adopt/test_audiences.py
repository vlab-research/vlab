...
def _responses(data):
    cols = ["question_ref", "response", "userid", "shortcode", "timestamp"]
    return pd.DataFrame(data, columns=cols)


def _resp(ref, response, userid, shortcode, day, hour=None):
    now = datetime.utcnow()
...
def test_partitioning_view_min_users_when_not_past():
    df = _responses(
        [
            _resp("one", "yes", "foo", "code1", 1),
            _resp("two", "yes", "foo", "code1", 1),
            _resp("one", "yes", "bar", "code1", 3),
            _resp("one", "yes", "baz", "code1", 4),
        ]
    )
...
def test_partitioning_view_min_users_when_past():
    ...
    assert set(a.userid.tolist()) == {"foo", "bar"}
    assert b.shape[0] == 1  # last user
    assert set(b.userid.tolist()) == {"baz"}
...
def test_partitioning_view_min_users_min_days_when_past_users_but_many_days():
    ...
    df = _responses(
        [
            _resp("one", "yes", "foo", "code1", 1),
            _resp("two", "yes", "foo", "code1", 1),
            _resp("one", "yes", "bar", "code1", 4),
            _resp("one", "yes", "baz", "code1", 5),
            _resp("one", "yes", "qux", "code1", 5),
            _resp("one", "yes", "quxx", "code1", 5),
        ]
    )
    ...
    a, b = partitioning_view(df, part, now)
    assert a.shape[0] == 5
    assert set(a.userid.tolist()) == {"foo", "bar", "baz", "qux"}
    assert b.shape[0] == 1
    assert set(b.userid.tolist()) == {"quxx"}
...
def test_partitioning_view_min_users_max_days_max_users_when_past_days_now():
    ...
    a, b = partitioning_view(df, part, now)
    assert a.shape[0] == 2
    assert set(a.userid.tolist()) == {"foo"}
    assert b.shape[0] == 0
...
def test_partitioning_view_min_users_max_days_max_users_when_past_days():
    ...
    df = _responses(
        [
            _resp("one", "yes", "foo", "code1", 1, 0),
            _resp("two", "yes", "bar", "code1", 2, 23),
            _resp("two", "yes", "baz", "code1", 3, 1),  # slightly more than 2 days.
        ]
    )
...
def test_partition_users_raises_if_not_valid_conf():
    conf = AudienceConf("foo", "PARTITIONED", ["foo"])
    with pytest.raises(Exception):
        partition_users(pd.DataFrame([]), conf)
...
def test_partition_users_partitions_by_user_ignores_extra():
    df = _responses(
        [
            ...
            _resp("one", "yes", "baz", "code1", 4),
            _resp("one", "yes", "qux", "code1", 5),
            _resp("one", "yes", "quxx", "code1", 6),
        ]
    )
    now = datetime(2021, 1, 10)

    part = Partitioning(min_users=2)
    conf = AudienceConf("foo", "PARTITIONED", ["foo"], partitioning=part)
...
def test_hydrate_audience_creates_partitioned_audiences():
    ...
    conf = AudienceConf("foo", "PARTITIONED", ["code1"], partitioning=part)
...
def test_hydrate_users_works_when_no_users_for_any_type():
    ...
    conf = AudienceConf(name="foo", subtype="CUSTOM", shortcodes=["code1"])
    audiences = hydrate_audience("page", df, conf, now)
    assert len(audiences) == 1
