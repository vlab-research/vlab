...
# ---------------------------------------------------------------------------
# There is deliberately NO provenance plumbing here any more.
#
# `Instruction` used to carry a 5th `provenance` field and `ad_dif`/`adset_dif`
# a trailing `provenance` param, so that a create instruction arrived at the
# imperative shell knowing which (stratum, creative) it was for and a mapping
# row could be written from the id Facebook returned.
#
# It was removed because it made reconciliation a *writer*. The row is written
# by `malaria.heal_ad_attributions`, which reads the ads that exist and joins
# them to `marketing.ad_provenance` on (adset name, ad name) -- so it does not
# need to have been present when the ad was made, and it covers ads this module
# never emitted an instruction for. That was the actual bug: every ad of
# vl-pulse-nigeria-smoke was created through the dashboard, and no amount of
# correctness here would have written its row.
#
# Reconciliation is back to being only about what Facebook should hold. If a
# future change wants to thread something through an Instruction to make a
# database write possible, this is the note saying that was tried.
# ---------------------------------------------------------------------------
