...
def multiple_choice(q):
    ...
    if labels[0] == "A":
        return [(lbl, grep_label(q["title"], lbl)) for lbl in labels]
...
def response_translator(q, qt=None):
    ...
    if ta != tb:
        raise TranslationError(
            f'Cannot translate. Questions with refs {q["ref"]} and '
            f'{qt["ref"]} across forms have different types: {ta} and {tb}'
        )

    try:
        lookup = {
            res: vt for (res, _), (_, vt) in zip(make_answers(q), make_answers(qt))
        }
    except TypeError:
...
def translate_response(translators, surveyid, q_ref, response):
    ...
    try:
        ...
        raise TranslationError(
            "Cannot find question ref {q_ref} in survey {surveyid}"
        ) from e
...
def _make_translators(ref_form, other_form):
    ...
    return {b["ref"]: response_translator(b, a) for a, b in fields}


def make_translators(forms):
    ts = [
        {
            form["id"]: _make_translators(ref_form["form_json"], form["form_json"])
...
def _get_ref(forms, field, ref):
    ref_forms = [f for f in forms if f["metadata"][field] == ref]

    try:
        latest = sorted(ref_forms, key=lambda f: f["created"])[-1]
    except IndexError as e:
        raise TranslationError(
            f"Could not find reference form from forms with metadatas "
            f'{[f["metadata"] for f in forms]}'
        ) from e
...
def org_translators(forms, conf):
    ...
    for f in forms:
        md = f["metadata"]
        key = frozenset([(k, v) for k, v in md.items() if k != field])
        try:
            d[key] += [f]
        except KeyError:
            d[key] = [f]

    return [(_get_ref(fs, field, conf["reference"]), fs) for fs in d.values()]
