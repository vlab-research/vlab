...
async function processor (message, consumer, eventStore) {
  ...
  try {
    ...
    // const form = getFormCached(state.form)
    // const user = await getUserCached(userId)
    // const action = getMessage({user, form }, state, event)
    // getMessage >> act(ctx, state, exec(state, event))
    // stateStore.updateState(userId, event)
    // updateState >> apply(state, exec(state, event))
