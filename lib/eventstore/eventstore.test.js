...
describe('EventStore', () => {
  it('returns just the event when db returns nothing', async () => {
    ...
    const events = await e.getEvents('foo', JSON.stringify({ messaging: [{baz: 'baz'}]}))
    events.should.deep.equal([ { baz: 'baz' }])
  })

  it('updates database when db returns something', async () => {
    const db = { get: sinon.fake.returns([JSON.stringify({ messaging: [{bar: 'bar'}]}),
                                          JSON.stringify({ messaging: [{baz: 'baz'}]})]) }
    const e = new s.EventStore(db)
    const events = await e.getEvents('foo', JSON.stringify({ messaging: [{baz: 'baz'}]}))
