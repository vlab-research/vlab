...
const { echo, tyEcho, statementEcho, repeatEcho, delivery, read, qr, text, sticker, multipleChoice, referral} = require('./events.test')
...
describe('getState', () => {
  ...
  xit('Responds when it gets external events that fulfills timeout conditions', () => {
    const wait = { type: 'timeout', value: '2 days' }

    // value should be...?
    const externalEvent = { source: 'synthetic',
                            type: 'external',
                            event: { type: 'timeout', value: 2843098034 }}
  ...
  xit('Responds when it gets external events that fulfill other conditions', () => {

    const wait = {
      operator: 'or',
      conditions:
      [ { type: 'timeout', value: '2 days' },
        { type: 'event', value: { type: 'moviehouse:startVideo', id: '90809843' } } ]
    }

    const externalEvent = { source: 'synthetic',
                             event: { type: 'moviehouse:startVideo', id: '809243}' }}
  ...
  xit('continues to wait when it gets external events that partially fulfill conditions', () => {
  ...
  // const externalEvent = { source: 'synthetic,
    ...
    const log = [referral, text, {...echo, message: {...echo.message, metadata: { wait: {foo: 'bar' }}}}, externalEvent]
  ...
  xit('Responds when it gets multiple events that fulfill all conditions', () => {
    ...
    const log = [referral, text, {...echo, message: {...echo.message, metadata: { wait: {foo: 'bar' }}}}, externalEventA, externalEventB]
