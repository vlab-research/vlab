...
const {getQA, getMessage, exec, act, apply, getState, getCurrentForm, getWatermark} = require('./machine')
...
describe('getQA', () => {
  ...
  it('returns [] if no log', () => {
    const qa = getQA()
    qa.should.deep.equal([])
  })
...
describe('Machine', () => {
  ...
  it('gets the correct start field even with no referral', () => {
    const output = exec({ state: 'START'}, text)
    const action = act({form, log:[text]}, [], {state: 'START' }, output)[0]
  ...
  it('sends the first message when it gets a referral', () => {
    ...
    const action = act({form, log:[referral]}, [], {state: 'START' }, output)[0]
    action.attachment.payload.text.should.equal(form.fields[0].title)
  })
  ...
  it('Works when initial messages are delivery', () => {
    ...
    output.action.should.equal('WAIT_DELIVERY')
