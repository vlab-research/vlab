...
describe('interpolateField', () => {
  ...
  it('Throws if the field is unanswered', () => {
    const ctx = {log: [referral, echo, qr, statementEcho], user: {}}
...
describe('jump', () => {
  ...
  it('it defaults to the next field if it cannot find an action', () => {
    ...
    f.jump({ form, log: []}, logic).ref.should.equal(form.fields[0].ref)
  })
})
...
describe('getCondition', () => {
  ...
  it('works with number equals and not equals is and is not- type casting!', () => {
    ...
    f.getCondition({ form, log: [echoBaz, delivery, read, num]}, '', {...cond, op: 'equal'}).should.be.true
    f.getCondition({ form, log: [echoBaz, delivery, read, num]}, '', {...cond, op: 'is_not'}).should.be.false
    f.getCondition({ form, log: [echoBaz, delivery, read, num]}, '', {...cond, op: 'not_equal'}).should.be.false
  })
  ...
  it('works with lower_equal_than operator on numbers', () => {
    ...
    f.getCondition({ form, log: [echoBaz, delivery, read, num]}, '', cond).should.be.true
    f.getCondition({ form, log: [echoBaz, delivery, read, num]}, '', {...cond, op: 'greater_equal_than'}).should.be.true
  })

  it('works with "and" and "or" operators', () => {

    const echoBaz = {...echo, message: { ...echo.message, metadata: { "ref": "baz"}}}
    const echoQux = {...echo, message: { ...echo.message, metadata: { "ref": "qux"}}}

    const msg1 = { postback: {payload: {value:true, ref: "baz"}, title: 'Yes' }}
    const msg2 = { postback: {payload: {value:true, ref: "qux"}, title: 'Yes' }}
    const msg2b = { postback: {payload: {value:false, ref: "qux"}, title: 'Yes' }}
    ...
    f.getCondition({ form, log: [echoBaz, msg1, echoQux, msg2]}, '', cond).should.be.true
    f.getCondition({ form, log: [echoBaz, msg1, echoQux, msg2]}, '', {...cond, op: 'or' }).should.be.true
    f.getCondition({ form, log: [echoBaz, msg1, echoQux, msg2b]}, '', cond).should.be.false
    f.getCondition({ form, log: [echoBaz, msg1, echoQux, msg2b]}, '', {...cond, op: 'or' }).should.be.true
  })
