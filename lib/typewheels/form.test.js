...
describe('interpolateField', () => {
  ...
  it('works with description', () => {
    const ctx = {log: [referral, text], user: { name: 'Foo Bazzle'}}
    const i = f.interpolateField(ctx, { description: 'name: {{hidden:name}}'})
    i.description.should.equal('name: Foo Bazzle')
  })

  it('encodes urls interpolation', () => {
    const ctx = {log: [referral, text], user: { name: 'Foo Bazzle'}}
    const i = f.interpolateField(ctx, { description: 'url: https://hello.com/?name={{hidden:name}}'})
    i.description.should.equal('url: https://hello.com/?name=Foo%20Bazzle')
  })
