...
class Machine {
  ...
  async run(state, user, event) {
    ...
    try {
      ...
      return { user, page, error: { tag: 'STATE_TRANSITION', message: e.message, stack: e.stack, state, event }}
    }
    try {
      ...
      if (e instanceof MachineIOError) {
        return {user, page, newState, error: { ...e.details, tag: e.tag, message: e.message, stack: e.stack }}
      } else {
        return {user, page, newState, error: { tag: 'STATE_ACTIONS', message: e.message, stack: e.stack }}
      }
    }
  }
}
