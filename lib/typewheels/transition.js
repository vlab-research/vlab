  ...
  async run(state, user, rawEvent) {
    ...
    try {
      ...
      if (output.action === 'NONE') {
        ...
        return { publish: false, timestamp, user, page, newState, payment }
      }

    } catch (e) {
      return { publish: false, timestamp, user, page, error: { tag: 'STATE_TRANSITION', message: e.message, stack: e.stack, state, event }}
    }
    try {
      ...
      return { publish: true, timestamp, user, page, actions, responses, payment, newState }

    } catch (e) {
      if (e instanceof MachineIOError) {
        return { publish: true, timestamp, user, page, payment, newState, error: { ...e.details, tag: e.tag, message: e.message, stack: e.stack }}
      } else {
        return { publish: true, timestamp, user, page, payment, newState, error: { tag: 'STATE_ACTIONS', message: e.message, stack: e.stack }}
      }
    }
  }
}
