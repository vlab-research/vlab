...
function getField({form}, ref, index=false) {
  if (!form.fields.length) {
  ...
  if (!field) {
    throw new FieldError(`Could not find the requested field, ${ref}, in our form: ${form.id}!`)
  }
...
function getCondition(ctx, qa, ref, {op, vars}){
  ...
  return fn(...vars.map((v)=> getVar(ctx, qa, ref, v, vars)))
}
