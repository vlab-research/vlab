...
function getLastSeenQuestion (log) {
  // Some users might not have "read" notification
  // turned on. Thus we use delivery watermark to
  // determine which question they are answering
  // NOTE: this might not be the best way to handle
  // that edge case?
  const deliveryWatermark = getWatermark('delivery', log)

  return log
    .filter(i => i.message && i.message.is_echo)
    .filter(i => i.timestamp <= deliveryWatermark )
...
function getFieldValue({log}, ref) {
  ...
  const ans = match && match[1].slice(-1)[0]
  if (!ans) return null

  if (ans.message && ans.message.quick_reply) return ans.message.quick_reply.payload.value
...
function getCondition(ctx, ref, {op, vars}){
  ...
  return fn(...vars.map(v => getVar(ctx, ref, v)))
}
