...
function act (ctx, qa, state, output) {
  switch(output.action) {

  case 'RESPOND':
    return respond(ctx, qa, state, output)
...
function respond (ctx, qa, state, output) {
...
function getQA(log) {
  ...
  const [,qa] = log.reduce(([s,qa],e) => {
    const output = exec(s,e)
    const state = apply(s, output)
    return [state, updateQA(qa, update(output))]
  }, [_initialState(), []])
...
function getMessage(log, form, user) {
  ...
  return act({ log, form, user }, qa, state, output)
}
