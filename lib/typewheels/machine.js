...
const {validator, defaultMessage}= require('@vlab-research/translate-typeform')
...
function _redo(event) {
  return (event.source === 'synthetic') && (event.event.type === 'redo')
}
...
function categorizeEvent(nxt) {
  if (nxt.referral ||
      ...
      (nxt.postback && nxt.postback.payload && nxt.postback.payload.referral)) {
    return 'REFERRAL'
  }

  if (_redo(nxt)) return 'REDO'
...
function exec (state, nxt) {
  ...
  case 'EXTERNAL_EVENT': {
    ...
    if (needsToken && state.tokens) {
      ...
      // TODO: token shouldn't be "removed" from tokens list
      // until echo!! That way failed send doesn't lose tokens
