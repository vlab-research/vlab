...
class Responser {
  ...
  async updateStore(userId, e) {
    ...
    if (update) {
      ...
      const {md} = newState
      const [seed, parentShortcode] = [md.seed, md.form]
      const [__, parentSurveyId] = this.machine.getForm(pageid, parentShortcode, parsedEvent)

      vals = [parentSurveyId, parentShortcode, surveyId,
              shortcode, flowId, userId,
              ref, qidx, title,
              value, seed, new Date(timestamp)]
    }
