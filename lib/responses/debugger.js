...
const {getPage} = require('./responser')
...
// TEMP
const typeform = require('../typewheels/typeform')
const ourform = require('../typewheels/ourform')
machine.getForm = (pageid, shortcode) => {
  return machine.cache.wrap(`form:${pageid}:${shortcode}`, () => {
    if (+shortcode > 0) {
      return ourform.getForm(pageid, shortcode)
    }
    return typeform.getForm(shortcode)
  }, '600s')
}
