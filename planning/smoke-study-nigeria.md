...
**Status:** **the survey and the study shell exist; nothing is configured.**
Verified in production 2026-08-27:

| | |
|---|---|
| `chatroach.surveys` | `vlpulseng` — "VL Pulse Nigeria - Smoke", created 2026-08-21 16:24 UTC |
| `vlab.studies` | `vl-pulse-nigeria-smoke`, `e460dfde-a010-49b3-b012-d48dbaadb34b`, created 2026-08-21 18:54 UTC |
| `study_confs` | **0 rows** |
| `study_state` | no row — `start_date` and `end_date` are NULL |

So it was created and then stopped, with no confs, never active, no ad, no spend.
It stalled on two things, **both now cleared**: ref mode was API-only and needed
an Auth0 bearer token (the dashboard control shipped `e7683333`, 2026-08-24 — see
`planning/ref-mode-dashboard-ux.md`), and the postgres MCP had died, blocking the
promised pre-flight that the `ad_attributions` rows and their `ref_token`s land
before adopt spends (that pre-flight is now
`adopt/scripts/write_path_probe.py` plus `planning/encoded-ref-probe-runbook.md`
§§4–6, over `kubectl` rather than the MCP).

**The procedure lives in `planning/encoded-ref-probe-runbook.md`.** This document
is the study *design* — instrument, consent, payment, parameters. The runbook is
how it gets fielded, and it splits the run into a free Messenger-only pass that
proves the attribution chain and a paid pass that adds the other two arms.
...
- ~~Whether to field all three arms at once or messenger+whatsapp first~~
  **Decided: Messenger alone first, and unpaid.** Two measurements settle it.
  (a) On 2026-08-17 a multi ad's WhatsApp arm was **never reachable by preview** —
  every attempt followed the single-valued `MESSAGE_PAGE` CTA to Messenger
  (`ctwa-probe-runbook.md` decision row G, still open), so that arm cannot be
  validated before spending. (b) WhatsApp is not a live transport
  (fly `planning/multi-platform-plan.md`). Fielding all three at once would put
  the one arm that can be verified for free behind the two that cannot.

  The concrete risk this avoids is unchanged: if the symmetry inference is wrong,
  those respondents land in `FALLBACK_FORM` 305, another researcher's live survey.
  `RecruitmentAdArrivalsInFallback` fires at ≥2 such arrivals in an hour.
  Runbook §§3–8 is pass 1 (Messenger, paused, free); §11 is pass 2.
