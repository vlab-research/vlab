...
Both are in `read_back()` (`adopt/scripts/ctwa_probe.py:495-565`), carry explanatory
comments in place, and are **uncommitted** on the vlab main worktree along with the rest of
`adopt/scripts/` (the directory is untracked). They matter more than their size: `--read-back`
