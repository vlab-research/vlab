...
Status: **Phase 0 and Phase 2 implemented; Phase 1 in review (PR #254);
Phases 3–4 not started.** §1–§5 are findings read out of the code, with the
file establishing each claim named next to it. §7 records the decisions taken
and why, §8 is the plan, §10 is what is still open, and **§11 (Phase 0) and
§13 (Phase 2) record what actually shipped and the claims the implementations
proved wrong** — several of them load-bearing, so read those before trusting a
